...
fn auto_wrap_procedure(stmt: String) -> String {
    ...
    let reserved_keywords = [
        "SELECT", "INSERT", "UPDATE", "DELETE", "CREATE", "ALTER", "DROP", "DECLARE", "SET", "IF",
        "BEGIN", "END", "WHILE", "FOR", "MERGE", "WITH", "UNION", "USE", "PRINT", "RETURN", "CAST",
        "CASE",
    ];
    ...
    let is_likely_procedure = trimmed
        .chars()
        .next()
    ...
    if is_likely_procedure {
        ...
        if trimmed_upper.contains(" WITH")
            || trimmed_upper.contains(" ORDER BY")
            || trimmed_upper.contains(" GROUP BY")
            || trimmed_upper.contains(" WHERE")
            || trimmed_upper.contains(" FROM")
        ...
        if !has_parentheses
            || trimmed_upper.starts_with("SP_")
            || trimmed_upper.starts_with("DBO.SP_")
...
mod tests {
    use super::{
        batch_needs_scope_preservation, infer_statement_kind, parse_sql_statements, StatementKind,
    };
...
";
    ...
    fn keeps_declare_batch_together_when_declare_leads() {
        ...
        assert_eq!(
            stmts.len(),
            1,
            "DECLARE-led batch must stay as one statement"
        );
    ...
    fn keeps_declare_batch_together_when_declare_is_in_the_middle() {
        ...
        assert_eq!(
            stmts.len(),
            1,
            "USE-prefixed DECLARE batch must stay as one"
        );
    ...
    fn keeps_temp_table_create_then_select_together() {
        ...
        assert_eq!(
            stmts.len(),
            1,
            "temp-table batch must stay as one statement"
        );
    ...
    fn scope_detects_declare_anywhere_outside_comments() {
        assert!(batch_needs_scope_preservation(
            "SELECT 1; DECLARE @x INT; SELECT @x;"
        ));
    }
    ...
    fn scope_detects_hash_temp_table_reference() {
        assert!(batch_needs_scope_preservation(
            "SELECT * INTO #t FROM Foo; SELECT * FROM #t;"
        ));
    ...
    fn scope_ignores_hash_inside_bracketed_identifier() {
        ...
        assert!(!batch_needs_scope_preservation(
            "SELECT [x] AS [#count] FROM T; SELECT 1;"
        ));
    }
...
impl QueryEngine {
    ...
    pub async fn execute_query(
        ...
        self.execute_batch(connection_id, statements, database, is_selection, max_rows)
            .await
    }
    ...
    async fn execute_single_statement(
        ...
        // Store cancel sender for this query
        {
            ...
            log_info!(
                "[QUERY] Stored cancel sender for query_id={}, total senders={}",
                query_id,
                senders.len()
            );
        ...
        log_info!(
            "[QUERY] Creating dedicated connection for query_id={}",
            query_id
        );
        let mut conn = self
            .connection_manager
            ...
            .await?;
        log_info!(
            "[QUERY] Dedicated connection created for query_id={}",
            query_id
        );
        ...
        let effective_max = max_rows.filter(|&m| m > 0).unwrap_or(5000).min(100_000);
        ...
        log_info!(
            "[QUERY] Executing query with tokio::select!, query_id={}",
            query_id
        );
        ...
        let query_future = async {
            log_info!(
                "[QUERY] Query future started, query_id={}",
                query_id_for_log
            );
            if is_dml {
                ...
                let affected_rows =
                    execute_result.rows_affected().iter().copied().sum::<u64>() as usize;
                ...
                log_info!(
                    "[QUERY] Query stream received, fetching results, query_id={}",
                    query_id_for_log
                );
                ...
                log_info!(
                    "[QUERY] Query results fetched, query_id={}",
                    query_id_for_log
                );
        ...
        // Remove cancel sender (query completed)
        {
            ...
            log_info!(
                "[QUERY] Removed cancel sender after completion, query_id={}",
                query_id
            );
        }
                ...
                if !is_dml {
                    ...
                    for (result_set_index, rows) in all_result_sets.iter().enumerate() {
                        ...
                        let columns: Vec<ColumnInfo> =
                            rows[0].columns().iter().map(ColumnInfo::from).collect();
    ...
    pub async fn cancel_query(&self, query_id: &str) -> bool {
        ...
        if let Some(cancel_tx) = senders.remove(query_id) {
            log_info!(
                "[CANCEL] Found cancel sender for query_id={}, sending cancel signal",
                query_id
            );
            // Send cancel signal - this will trigger the select! branch
            match cancel_tx.send(()) {
                Ok(_) => log_info!(
                    "[CANCEL] Cancel signal sent successfully for query_id={}",
                    query_id
                ),
                Err(_) => log_warn!(
                    "[CANCEL] Failed to send cancel signal (receiver dropped) for query_id={}",
                    query_id
                ),
            }
    ...
    pub async fn cancel_all_for_connection(&self, connection_id: &str) -> usize {
        log_info!(
            "[CANCEL] cancel_all_for_connection called with connection_id={}",
            connection_id
        );
        ...
        let query_ids: Vec<String> = {
            ...
            info.iter()
                .filter(|(_, qi)| {
                    qi.connection_id == connection_id && qi.status == QueryStatus::Running
                })
                .map(|(id, _)| id.clone())
                .collect()
        };
        ...
        log_info!(
            "[CANCEL] Found {} running queries for connection_id={}: {:?}",
            query_ids.len(),
            connection_id,
            query_ids
        );
        ...
        log_info!(
            "[CANCEL] Cancelled {} queries for connection_id={}",
            cancelled_count,
            connection_id
        );
        cancelled_count
    ...
    pub async fn cleanup_old_queries(&self) {
        let mut info = self.query_info.write().await;
        info.retain(|_, qi| qi.status == QueryStatus::Running || qi.status == QueryStatus::Pending);
    }
}
